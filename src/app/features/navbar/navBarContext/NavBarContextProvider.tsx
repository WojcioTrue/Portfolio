...
const NavBarContext = createContext<{
  ...
  defaultIndicatorRef:
    | React.RefObject<HTMLSpanElement | HTMLDivElement>
    | undefined;
  skillsIndicatorRef:
    | React.RefObject<HTMLSpanElement | HTMLDivElement>
    | undefined;
...
const NavBarContextProvider = ({ children }: ChildrenType) => {
  ...
  //indicators Ref
  const defaultIndicatorRef = useRef<HTMLSpanElement | HTMLDivElement | null>(
    null,
  );
  const skillsIndicatorRef = useRef<HTMLSpanElement | HTMLDivElement | null>(
    null,
  );
  const aboutIndicatorRef = useRef<HTMLSpanElement | HTMLDivElement | null>(
    null,
  );
  const textIndicatorRef = useRef<HTMLSpanElement | HTMLDivElement | null>(
    null,
  );
  const somethingIndicatorRef = useRef<HTMLSpanElement | HTMLDivElement | null>(
    null,
  );
  // menu li refs
  ...
  const [navBarDesktopPosition, setNavBarDesktopPosition] =
    useState<ItemPositionType>({
      left: 0,
      right: 0,
      top: 0,
      bottom: 0,
    });
  const [navBarMobilePosition, setNavBarMobilePosition] =
    useState<ItemPositionType>({
      left: 0,
      right: 0,
      top: 0,
  ...
  const [iDesktopPosition, setIDesktopPosition] =
    useState<IndicatorDesktopType>({
      horizontalMid: 0,
      verticalMid: 23,
    });
