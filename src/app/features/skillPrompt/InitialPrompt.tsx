...
const InitialPrompt = () => {
  const { promptProps, promptSkill, startResize } =
    useContext(SkillPromptContext);
  ...
  return (
    <div
      ...
      className="relative w-[90%] overflow-hidden rounded-[7px] opacity-0 sx:w-[350px]"
    >
      <div className="relative h-[150px] w-[100%] bg-red-500">
        <div className="absolute left-[50%] top-[25%] h-[50px] w-[50px] bg-blue-500">
          <Image className="" src={skill.src} alt="#" fill />
        </div>
      </div>
      <div id="skill-text" className="mx-6 py-[30px] text-white">
        <p className="text-4xl">{skill.name}</p>
        <div className="my-3 h-[1px] w-full bg-my-lightBackground"></div>
        <p>{skill.text}</p>
      </div>
    </div>
  );
